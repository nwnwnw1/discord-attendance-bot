...
- `出勤` ボタン: 出勤中でなければ現在時刻を記録し、出勤中であることを表示
- `退勤` ボタン: 出勤中の記録に現在時刻を記録し、退勤済みであることを表示。退勤後にもう一度押した場合は後の時刻で更新
...
退勤ボタンを押し忘れた場合、その出勤記録は翌朝5時以降に `未打刻` として扱われます。
未打刻扱いになった後は、新しい出勤を記録できます。
退勤後に追加で作業して再度退勤ボタンを押した場合は、同じ勤務日の範囲内で後から押した退勤時刻に更新されます。

## 月別一覧

一覧専用パネルの `勤怠一覧` ボタン、または `/勤怠一覧` で自分の指定月の勤怠を表示できます。
月は `YYYY-MM` 形式で指定します。月を省略した場合は当月の一覧を表示します。
一覧には曜日が表示され、勤怠がない日も含めてその月の全日付が表示されます。
一覧を表示した後にも `勤怠一覧` ボタンが表示されるため、続けて別の月を確認できます。
